--[[
    PalInfiniteAmmo
    Maintient les munitions au maximum pour toutes les armes.
]]

local MOD_NAME = "PalInfiniteAmmo"
...
local function loadConfig()
    ...
    Config = { modName = MOD_NAME, enabled = true, keybind = "F7", intervalMs = 200, debug = false }
end
loadConfig()
...
local function refillAmmo()
    ...
    for _, item in ipairs(items) do
        if isValid(item) then
            local maxAmmo = safeGet(item, "MaxBulletCount") or safeGet(item, "MaxAmmo")
            if maxAmmo then
                pcall(function() item.BulletCount = maxAmmo end)
                pcall(function() item.AmmoCount = maxAmmo end)
            end
        end
    end
end
...
log("PalInfiniteAmmo charge. Touche: " .. tostring(Config.keybind))
