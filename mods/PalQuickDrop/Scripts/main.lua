--[[
    PalQuickDrop
    Depose rapidement un objet de l'inventaire via un raccourci clavier.
--]]

local MOD_NAME = "PalQuickDrop"
...
local function onQuickDrop()
    ...
    if itemId and itemId ~= "" then
        -- Essayer de deposer par ID
        local ok = tryCall(target, {
            "DropItem", "ServerDropItem", "DropItemToFloor", "DropInventoryItem", "ServerDropInventoryItem"
        }, itemId, amount)
        if ok then
            log("Depose : " .. itemId .. " x" .. amount)
            return
        end
    else
        -- Essayer par slot selectionne
        local ok = tryCall(target, {
            "DropSelectedItem", "ServerDropSelectedItem", "DropItemBySlot", "ServerDropItemBySlot", "DropSlot"
        }, slot, amount)
        if ok then
            log("Depose slot " .. slot .. " x" .. amount)
            return
        end
    end

    err("Aucune methode de depot disponible.")
end
...
log("charge avec succes. Appuyez sur " .. (Config.keybind or "F6") .. " pour deposer rapidement.")
