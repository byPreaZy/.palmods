...
namespace Engine
{
    ...
    // ------------------------------------------------------------------------
    // Résolution du joueur local via GWorld chain
    // ------------------------------------------------------------------------

    static void* GetGWorld()
    {
        if (!g_Base) return nullptr;
        uintptr_t rva = PalTrainerRuntime::GWorldRva ? PalTrainerRuntime::GWorldRva : PalTrainerOffsets::GWorldRva;
        void** ptr = (void**)(g_Base + rva);
        if (!IsValidPointer((uintptr_t)ptr)) return nullptr;
        return *ptr;
    }
    ...
    void* GetLocalPlayerController()
    {
        ...
        void* gameInstance = *(void**)((uintptr_t)world + PalTrainerOffsets::UWorld_OwningGameInstance);
        if (!gameInstance) return nullptr;

        SDK::TArray<void*>* localPlayers = (SDK::TArray<void*>*)((uintptr_t)gameInstance + PalTrainerOffsets::UGameInstance_LocalPlayers);
        if (!localPlayers || localPlayers->Count <= 0 || !localPlayers->Data) return nullptr;

        void* localPlayer = localPlayers->Data[0];
        if (!localPlayer) return nullptr;

        void* controller = *(void**)((uintptr_t)localPlayer + PalTrainerOffsets::UPlayer_PlayerController);
        return controller;
    }

    SDK::APalCharacter* GetLocalPlayerPawn()
    {
        void* controller = GetLocalPlayerController();
        if (!controller) return nullptr;

        void* pawn = *(void**)((uintptr_t)controller + PalTrainerOffsets::APlayerController_AcknowledgedPawn);
        if (!pawn)
            pawn = *(void**)((uintptr_t)controller + PalTrainerOffsets::AController_Pawn);
        if (!pawn) return nullptr;

        return (SDK::APalCharacter*)pawn;
    }

    SDK::APalPlayerState* GetLocalPlayerState()
    {
        void* controller = GetLocalPlayerController();
        if (!controller) return nullptr;

        void* playerState = *(void**)((uintptr_t)controller + PalTrainerOffsets::AController_PlayerState);
        if (!playerState) return nullptr;

        return (SDK::APalPlayerState*)playerState;
    }
}
