...
#endif
static bool g_showMenuWindow = false;
...
static ImVec4 AnimColorLerp(const ImVec4& current, const ImVec4& target, float speed) {
    float dt = ImGui::GetIO().DeltaTime;
    float t = 1.0f - std::exp(-speed * dt);
    return ImVec4(
...
static ImU32 AnimPulse(float time, float speed, float min, float max) {
    float v = min + (max - min) * (0.5f + 0.5f * std::sin(time * speed));
    return IM_COL32((int)(v * 255), (int)(v * 255), (int)(v * 255), 255);
}

// Variables d'animation
static float g_animSidebarWidth = 240.0f;
static float g_animZoom = 1.0f;
static float g_animCenterX = 0.5f;
static float g_animCenterY = 0.5f;
static float g_animTime = 0.0f;

// Couleurs animées pour boutons
struct AnimButtonState {
    ImVec4 hoverColor = ImVec4(0.14f, 0.14f, 0.16f, 1.0f);
    ImVec4 targetColor = ImVec4(0.14f, 0.14f, 0.16f, 1.0f);
};
static std::map<std::string, AnimButtonState> g_animButtons;

static ImVec4 GetAnimatedButtonColor(const char* id, bool hovered, bool active) {
    auto& state = g_animButtons[id];
    ImVec4 base(0.14f, 0.14f, 0.16f, 1.0f);
    ImVec4 hover(0.22f, 0.20f, 0.14f, 1.0f);
    ImVec4 activeCol(0.30f, 0.26f, 0.12f, 1.0f);
    state.targetColor = active ? activeCol : (hovered ? hover : base);
    state.hoverColor = AnimColorLerp(state.hoverColor, state.targetColor, 12.0f);
...
static ImVec2 g_menuWindowSize(260.0f, 600.0f);
...
static void RenderMiniMap() {
    ...
    // --- Fenêtre 1: Menu (fenêtre indépendante et déplaçable) ---
    {
        ...
        if (ImGui::BeginTabBar("##menu_tabs", ImGuiTabBarFlags_FittingPolicyScroll | ImGuiTabBarFlags_NoTooltip)) {

            // Onglet Carte
            if (ImGui::BeginTabItem("Carte")) {
                ImGui::Spacing();
                bool changed = false;
                ...
                ImGui::TextDisabled("  F1: Suivre joueur");
                ImGui::TextDisabled("  F2/F3: Zoom +/-");
                ImGui::TextDisabled("  F4: Toujours au-dessus");
                ImGui::TextDisabled("  F5: Afficher pals");
                ImGui::TextDisabled("  F6: Lignes ESP");
                ImGui::TextDisabled("  Clic-droit: Teleporter");
            ...
            if (ImGui::BeginTabItem("Spawns")) {
                ...
                if (!g_palSpawnsLoaded) {
                    ...
                    // Pal name list (filtered by search)
    ...
    // --- Fenêtre 2: Carte (fenêtre indépendante et déplaçable) ---
    {
        ImGui::SetNextWindowPos(g_mapWindowPos, ImGuiCond_Once);
        ImGui::SetNextWindowSize(g_mapWindowSize, ImGuiCond_Once);
        ImGui::PushStyleVar(ImGuiStyleVar_WindowPadding, ImVec2(0, 0));
        ImGui::Begin("Carte", &g_mapWindowOpen, ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoNav);
