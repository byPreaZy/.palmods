    ...
    bool Load(const char* jsonPath) {
        ...
        for (auto& kv : pairs) {
            try {
                ...
                if (kv.first == "GWorldRva")      GWorldRva = v;
                else if (kv.first == "GObjectRva")     GObjectRva = v;
                else if (kv.first == "FNameRva")       FNameRva = v;
                else if (kv.first == "ProcessEventRva") ProcessEventRva = v;
                else if (kv.first == "AppendStringRva") AppendStringRva = v;
                else if (kv.first == "TickRva")        TickRva = v;
            } catch (...) {}
        }
        return GWorldRva != 0;
    }
...
// ---------------------------------------------------------------------------
// Helpers de log (debug uniquement)
// ---------------------------------------------------------------------------
...
static void UpdateCheatState() {
    ...
    auto pairs = ParseSimpleJson(cmd);
    {
        ...
        for (auto& kv : pairs) {
            ...
            else if (k == "teleportToX") { g_cheats.teleportToX = ToFloat(v, 0.0f); g_cheats.teleportTo = true; consumed = true; }
        ...
        if (consumed) {
            // Vider le fichier de commandes pour que les actions one-shot ne se répètent pas à moins d'être renvoyées
...
// Helpers mémoire
// ---------------------------------------------------------------------------
static void* GWorldPtr() {
    void* base = GetModuleBase();
    if (!base) return nullptr;
    void** ptr = (void**)((uintptr_t)base + PalTrainerRuntime::GWorldRva);
    if (!ptr) return nullptr;
    return *ptr;
}

static void* GetFirstLocalPlayer(void* world) {
    if (!world) return nullptr;
    void* gameInstance = deref(world, PalTrainerOffsets::UWorld_OwningGameInstance);
    if (!gameInstance) return nullptr;
    TArray<void*>* localPlayers = (TArray<void*>*)((uintptr_t)gameInstance + PalTrainerOffsets::UGameInstance_LocalPlayers);
    if (!localPlayers || localPlayers->Count <= 0) return nullptr;
    return localPlayers->Data[0];
}

static void* GetPlayerController(void* world) {
    void* localPlayer = GetFirstLocalPlayer(world);
    if (!localPlayer) return nullptr;
    return deref(localPlayer, PalTrainerOffsets::UPlayer_PlayerController);
}

static void* GetPlayerPawn(void* controller) {
    if (!controller) return nullptr;
    // Essayer AcknowledgedPawn d'abord (client), puis Pawn
    void* pawn = deref(controller, PalTrainerOffsets::APlayerController_AcknowledgedPawn);
    if (!pawn) pawn = deref(controller, PalTrainerOffsets::AController_Pawn);
    return pawn;
}

static void* GetPlayerState(void* controller) {
    if (!controller) return nullptr;
    return deref(controller, PalTrainerOffsets::AController_PlayerState);
}
...
// Helpers FFixedPoint64 : la valeur est stockée en int64 ; échelle inconnue (souvent x1000)
static float FixedToFloat(FFixedPoint64 v) {
    return (float)v.Value / 1000.0f;
}

static void SetFixedPoint(void* ptr, FFixedPoint64 v) {
    if (!ptr) return;
    *(FFixedPoint64*)ptr = v;
}
...
// ---------------------------------------------------------------------------
static void ApplyCharacterMovementCheats(void* movement) {
    if (!movement) return;
    std::lock_guard<std::mutex> lock(g_cheatMutex);
    if (g_cheats.superSpeed) {
        float baseSpeed = g_cheats.speedValue;
        write<float>(movement, PalTrainerOffsets::UCharacterMovement_MaxWalkSpeed, baseSpeed);
        write<float>(movement, PalTrainerOffsets::UCharacterMovement_MaxSwimSpeed, baseSpeed);
        write<float>(movement, PalTrainerOffsets::UCharacterMovement_MaxFlySpeed, baseSpeed);
        write<float>(movement, PalTrainerOffsets::UCharacterMovement_MaxAcceleration, baseSpeed);
    }
    if (g_cheats.superJump) {
        write<float>(movement, PalTrainerOffsets::UCharacterMovement_JumpZVelocity, g_cheats.jumpValue);
    }
    if (g_cheats.flyMode) {
        write<float>(movement, PalTrainerOffsets::UCharacterMovement_MaxFlySpeed, g_cheats.speedValue);
    }
    if (g_cheats.noClip) {
        // Le mode vol active aussi un comportement proche du no-clip avec grande vitesse + faible gravité
        write<float>(movement, PalTrainerOffsets::UCharacterMovement_GravityScale, 1.0f);
    }
}

static void ApplyParameterCheats(void* charParamComp) {
    if (!charParamComp) return;
    std::lock_guard<std::mutex> lock(g_cheatMutex);
    if (g_cheats.godMode) {
        // Mettre les flags debug muteki à true
        // UPalCharacterParameterComponent::bIsDebugMuteki à 0x00CA
        *(uint8_t*)((uintptr_t)charParamComp + 0xCA) = 1;
        // UPalCharacterParameterComponent::IsImmortality à 0x538
        *(uint8_t*)((uintptr_t)charParamComp + 0x538) = 1;
    }
    void* individual = GetIndividualParameter(charParamComp);
    if (individual) {
        FFixedPoint64 maxHp = read<FFixedPoint64>(individual, PalTrainerOffsets::UPalIndividual_MaxHP);
        FFixedPoint64 maxSp = read<FFixedPoint64>(individual, PalTrainerOffsets::UPalIndividual_MaxMP);
        // MaxSP est en fait le champ MaxSP ; s'il est nul, fallback vers MaxMP (même pool en jeu)
        if (maxSp.Value == 0) maxSp = read<FFixedPoint64>(individual, PalTrainerOffsets::UPalIndividual_MaxSP);
        if (g_cheats.infiniteHP) {
            SetFixedPoint((void*)((uintptr_t)individual + PalTrainerOffsets::UPalIndividual_HP), maxHp);
        }
        if (g_cheats.infiniteSP) {
            // Le PS individuel est MaxSP, et le PS du composant est à 0x2F0
            SetFixedPoint((void*)((uintptr_t)individual + PalTrainerOffsets::UPalIndividual_MaxSP), maxSp);
            SetFixedPoint((void*)((uintptr_t)charParamComp + PalTrainerOffsets::UPalCharacterParameter_SP), maxSp);
        }
    }
}

static void ApplyInventoryCheats(void* inventory) {
    if (!inventory) return;
    std::lock_guard<std::mutex> lock(g_cheatMutex);
    if (g_cheats.infiniteWeight) {
        float desired = g_cheats.weightValue;
        write<float>(inventory, PalTrainerOffsets::UPalPlayerInventoryData_NowItemWeight, desired);
        float maxWeight = read<float>(inventory, PalTrainerOffsets::UPalPlayerInventoryData_MaxInventoryWeight);
        if (maxWeight < 99999.0f) {
            write<float>(inventory, PalTrainerOffsets::UPalPlayerInventoryData_MaxInventoryWeight, 9999999.0f);
            write<float>(inventory, PalTrainerOffsets::UPalPlayerInventoryData_MaxInventoryWeight + 0x04, 9999999.0f); // maxInventoryWeight_Cached à +4
        }
    }
}
...
// Lecture du statut
// ---------------------------------------------------------------------------
static PlayerSnapshot ReadPlayerSnapshot(void* world) {
    PlayerSnapshot snap;
    if (!world) return snap;
    void* controller = GetPlayerController(world);
    if (!controller) return snap;
    void* pawn = GetPlayerPawn(controller);
    if (!pawn) return snap;
    ...
    void* playerState = GetPlayerState(controller);
    if (playerState) {
    ...
    if (charParam) {
        ...
        if (individual) {
            ...
            snap.level = read<int32_t>(individual, 0x2A0 + 0x020); // Niveau dans SaveParameter
        }
    }
    snap.name = "Player";
    return snap;
...
static void WorkerLoop() {
    ...
    while (g_running) {
        ...
        void* world = GWorldPtr();
        PlayerSnapshot snap = ReadPlayerSnapshot(world);
        if (snap.valid) {
            void* controller = GetPlayerController(world);
            void* pawn = controller ? GetPlayerPawn(controller) : nullptr;
            if (pawn) {
                void* movement = GetCharacterMovement(pawn);
                void* root = GetRootComponent(pawn);
                void* charParam = GetPalCharacterParameterComponent(pawn);
                void* playerState = GetPlayerState(controller);
                void* inventory = playerState ? GetInventoryData(playerState) : nullptr;

                ApplyCharacterMovementCheats(movement);
                ApplyParameterCheats(charParam);
                ApplyInventoryCheats(inventory);
