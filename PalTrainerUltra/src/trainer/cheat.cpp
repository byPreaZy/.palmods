...
namespace Cheat
{
    ...
    static void ApplyDebugSettings()
    {
        ...
        *ds->GetWorkExtraRate() = (MassiveWorkSpeedPlayer || MassiveWorkSpeedAll) ? WorkSpeedRate : 1.0f;
    ...
    static void ApplyItemAmounts()
    {
        ...
        for (int32 c = 0; c < arr->NumChunks; ++c)
        {
            ...
            for (int32 i = 0; i < perChunk && (c * perChunk + i) < arr->NumElements; ++i)
            {
                ...
                SDK::UPalItemSlot* slot = (SDK::UPalItemSlot*)obj;
                if (ItemAmount >= 1 && IsOwnedByPlayer(obj))
